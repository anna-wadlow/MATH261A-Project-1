...
For the purpose of this paper, we use the DOB reported by ACS as a reasonable estimate. After data cleaning, the median age is `r round(summary_stats$median_age_years,2)` years, with minimum age `r round(summary(dogs_clean$age_years)[1],2)` years and maximum age `r round(summary(dogs_clean$age_years)[6],2)` years. 

The model's dependent variable, length of stay, is the difference in days between the outcome date and intake date. After data cleaning, the median length of stay is `r round(summary_stats$median_los_days,2)` days, with minimum LOS `r round(summary(dogs_clean$los_days)[1],2)` days and maximum LOS `r round(summary(dogs_clean$los_days)[6],2)` days.
...
```{r fig-dogs-scatter}
#| fig-cap: "Scatterplot of dog age at intake versus their length of stay for adopted dogs. The fitted linear model, y = .85x + 18) indicates a slight positive association."
#| echo: false
#| message: false
dogs_clean %>%
...
We use the base R's `lm()` function to implement the linear model and the `confint()` function to calculate confidence intervals  [@R_language] These are used alongside functions from the `dplyr` [@dplyr] and `tidyverse` [@tidyverse] packages for data cleaning and visualization.

Model fit was evaluated using diagnostic plots discussed in @sec-results.

# Results {#sec-results}

The estimated slope parameter is $b_1=$ `r round(m1$coefficient[2],3)`, which can be interpreted to mean that a one-year increase in dog's age, on average, corresponds to ~`r round(m1$coefficient[2],3)` additional days in LOS. A 95% confidence interval for the estimated slope parameter $b_1$ is about [`r round(confint(m1, "age_years", level = .95)[1],3)`, `r round(confint(m1, "age_years", level = .95)[2],3)`]. We estimate that 95% of the time, the true population slope is contained within this range.
The estimated intercept is $b_0=$ `r round(m1$coefficient[1],3)`, the expected LOS for a 0-year-old dog.

The results of the linear regression model should be approached cautiously since the residual plots @fig-diagnostics show important assumption violations. 
...
Finally, the QQ plot @fig-qq shows pronounced right‑skew and heavy tails, indicating residual non‑normality. A potential cause for this is that both our predictor and response variables are bounded at zero.

While the p-value for the $b_1$ estimator is small (`r m1_summary$coefficients[2,"Pr(>|t|)"]`), the $R^2$ value, `r round(m1_summary$r.squared,4)`, combined with the violations of necessary assumptions cast serious doubt on the validity of this model and any associated tests.

```{r fig-diagnostics}
#| fig-cap: "Residual diagnostic plots for linear regression model predicting a dog's length of stay based on their age at intake."
#| echo: false
#| message: false
...
```{r fig-qq}
#| fig-cap: "Diagnostic quantile-quantile plot indicates deviation from normality."
#| echo: false
#| message: false
